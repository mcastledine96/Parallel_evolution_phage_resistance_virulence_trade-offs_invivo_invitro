...
# also save out unknown genes dataset
ancestor_genes_unknown <- select(d_genes_unknown, ancestral_prop, pos, gene_name, ref, alt) %>%
  distinct(., .keep_all = TRUE) %>%
  pivot_longer(., ancestral_prop, names_to = 'treat', values_to = 'af')

d_genes_unknown <- unite(d_genes_unknown, 'id', c(treat, rep), na.rm = TRUE, remove = FALSE) %>%
  mutate(., id2 = as.numeric(as.factor(id)),
         change2 = ifelse(is.na(change2), 'ancestor', change2))

d_genes_unknown <- unite(d_genes_unknown, 'id3', c(gene_name, pos), na.rm = TRUE, remove = FALSE)

# save out as a table
write_csv(d_genes_unknown, 'data/sequencing/invitro_snps_indels_unknown_genes.csv')
...
#--------------------------------------------------------------------------#
# do genomic analyses on the genetic changes in unknown and known genes ####
#--------------------------------------------------------------------------#

# bind known and unknown genes together
d_genes_both <- bind_rows(d_genes_known, d_genes_unknown)

# clustering first
head(d_genes_both)

# setup data for clustering
d_ancest = bind_rows(ancestor, ancestor_genes_unknown) %>%
  mutate(., treat = 'ancestor',
                  rep = '')

d_clust <- select(d_genes_both, treat, rep, pos, ref, alt, af, gene_name) %>%
  bind_rows(., d_ancest) %>%
  unite(., 'id', c(treat, rep), sep ='.') %>%
  spread(., id, af) %>%
  arrange(pos) %>%
...
# vars
d_vars <- select(d_genes_both, gene_name, change2, change, pos, alt, ref, alt, ancestral_prop) %>%
  distinct() %>%
  group_by(gene_name) %>%
  mutate(n = 1:n()) %>%
  ungroup() %>%
...
# plot extra nmds plot

p_nmds <- ggplot() +
  geom_segment(aes(x = 0, y = 0, yend = nmds2, xend = nmds1, group = score, col = change2), d_gene, arrow = arrow(length = unit(0.01, "npc")), alpha = 0.5) +
  ggforce::geom_mark_hull(aes(nmds1, nmds2, group = treat), filter(d_sample, treat != 'ancestor')) +
  geom_point(aes(nmds1, nmds2, shape = treat), size = 5, fill = 'white', data = filter(d_sample, treat != 'ancestor')) +
  geom_point(aes(nmds1, nmds2, shape = treat), size = 6, fill = '#7090A0', data = filter(d_sample, treat == 'ancestor')) +
  scale_x_continuous(expand = c(.25, .25)) +
  scale_y_continuous(expand = c(.25, .25))  +
  labs(title = '(a) NMDS of euclidean distances between genome changes of known and unknown function',
       x = 'nmds 1',
       y = 'nmds 2',
       col = "Change type",
       shape = "Treatment") +
...
# 1. calculate distance from ancestor
d_snp_summary <- d_genes_both %>%
  group_by(treat, rep) %>%
  summarise(dist_ancest = sum(abs(diff)), .groups = 'drop')
d_snp_summary2 <- d_genes_both %>%
  filter(change2 != 'existing') %>% 
  filter(af > 0) %>%
  group_by(treat, rep) %>%
  tally() %>%
...
p2 <- ggplot(d_snp_summary, aes(treat, new_snps_indels)) +
  ...
  scale_shape_manual(values = c(22:24)) +
  scale_x_discrete(labels = c('Control', 'Phage\nadded\nonce', 'Phage\nadded\nrepeatedly')) +
  scale_y_continuous(limits = c(0, 120), n.breaks = 6)

# alpha diversity - heterozygosity

# calculate Hardy Weinberg equilibrium for each SNP
alpha_div <- mutate(d_genes_both, p = af,
                    q = 1-p,
                    div = 1 - p^2 - q^2) %>%
  group_by(., treat, rep) %>%
  summarise(., diversity = sum(div), .groups = 'drop')
...
ggsave('plots/Figure_S3.pdf', patchwork, width = 11, height = 10)
ggsave('plots/Figure_S3.png', patchwork, width = 11, height = 10)
