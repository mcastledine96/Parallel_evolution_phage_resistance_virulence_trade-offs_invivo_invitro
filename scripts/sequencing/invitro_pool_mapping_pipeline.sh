#------------------------------------------------------------------------------#
# processing and variant calling of pool-seq data from in vitro experiments ####
#------------------------------------------------------------------------------#
...
# install packages
conda install -c bioconda freebayes fastqc bcftools samtools bwa samblaster vcflib
...
# assign reference - Make sure it is there
...
for file in $trimmed_files/*R1.fastq.gz
...
for bam_file in $wd/processed/bams/*.bam
    ...
    do
        ...
        samtools flagstat $bam_files/mapped/${stub2}_refpa14_sorted.bam > $wd/processed/bams/stats/${stub2}_refpa14_mapped_stats.txt
        
        # raw bam file
        samtools flagstat $bam_file > $wd/processed/bams/stats/${stub2}_refpa14_raw_stats.txt
...
for bam_file in $wd/processed/bams/mapped/*sorted.bam
    ...
    do
        ...
        stub=${bam_file%.bam}
        stub2=$(basename ${bam_file%.bam})
        # call SNPs using FreeBayes, set ploidy to 24 (number of clones)
        freebayes -f $ref_pa14 -p 24 --use-best-n-alleles 4 --pooled-discrete $bam_file > $wd/processed/vcf_output/freebayes/${stub2}_freebayes.vcf
